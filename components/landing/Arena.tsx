"use client";
// =====================================================================
// AERARIUM landing — The Arena (head-to-head liquidation scoreboard)
// Ported from design/aer-arena.jsx. Reads ArenaStats from MarketSource.
// =====================================================================
import { useRef } from "react";
import type { ArenaStats } from "@/lib/market/MarketSource";
import { Section, Meander, useInView, PreviewBadge, ChainGlyph } from "./primitives";
import { CHAIN, fmtCompact, type Side } from "./tokens";
...
const Combatant = ({ chain, leading, align }: { chain: Side; leading: boolean; align: "left" | "right" }) => {
  const c = CHAIN[chain];
  return (
    <div style={{
      padding: "28px 32px",
...
const TugBar = ({ solPct }: { solPct: number }) => {
...
const StatRow = ({ label, evm, sol, money, suffix = "", last }: {
  label: string; evm: number; sol: number; money?: boolean; suffix?: string; last?: boolean;
}) => {
  const evmWins = evm > sol;
  const fmt = (v: number) => (money ? fmtCompact(v) : v.toLocaleString("en-US") + suffix);
  return (
    <div style={{
    ...
    <Section id="arena" style={{ paddingTop: 96, paddingBottom: 96 }}>
      <div style={{ textAlign: "center", marginBottom: 14 }}>
        ...
        <div className="aer-eyebrow" style={{ marginBottom: 16, display: "inline-flex", alignItems: "center", gap: 12 }}>
          The Arena · Last 30 days
          {arena.illustrative && <PreviewBadge />}
        </div>
