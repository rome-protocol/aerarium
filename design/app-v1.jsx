...
function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'SOL_TX_CONFIRM':
      return { ...state, solState: 'tx-confirm' };
    case 'SOL_TX_SUCCESS': {
...
function jumpSol(state, target) {
  ...
  switch (target) {
    ...
    case 'connected':
      return { ...state, solState: 'connected', ctx: { ...baseCtx, solWalletUsdc: 100, solPosition: { supplied: 0, earned: 0 } } };
    case 'tx-sign':
      return { ...state, solState: 'tx-sign' };
    case 'tx-confirm':
      return { ...state, solState: 'tx-confirm' };
    case 'tx-error':
      return { ...state, solState: 'tx-error' };
    case 'has-position':
      return {
      ...
      <TweakSection title="Solana lane state">
        <TweakSelect
          ...
          options={[
            ...
            { value: 'connected', label: '3b · Connected, no position' },
            { value: 'tx-sign', label: '3c · Awaiting signature' },
            { value: 'tx-confirm', label: '3c · Confirming on Solana' },
            { value: 'has-position', label: '3d · Has supply position' },
            { value: 'tx-error', label: '3f · Tx error' },
          ]}
        />
      </TweakSection>
...
const App = () => {
  ...
  // Auto-advance Solana tx sign → confirm → success
  useEffect(() => {
    if (state.solState === 'tx-sign') {
      const t = setTimeout(() => dispatch({ type: 'SOL_TX_CONFIRM' }), 1600);
      return () => clearTimeout(t);
    }
    if (state.solState === 'tx-confirm') {
      const t = setTimeout(() => dispatch({ type: 'SOL_TX_SUCCESS' }), 2000);
      return () => clearTimeout(t);
    }
  }, [state.solState]);
